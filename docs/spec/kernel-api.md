...
(The lazy-trigger names are deliberately not designed in v1; this field is the slot it will occupy, since with imperative registration future lazy activation must be driven entirely by manifest pre-declaration.)
...
The `diagnostics` surface, which covers emitting on and subscribing to the kernel's diagnostics channel, is defined in the diagnostics spec ([§6.2](./diagnostics.md#62-the-plugin-surface-apidiagnostics)).
...
There are no registry permissions (`commands:register` and the like), because registering primitives is what a plugin *is*, and no `bridge:storage` (§13.6).
...
The application developer passes a storage engine at kernel construction, through the `storage` option of `createSwitchboard` (§18.1). There is one engine per kernel instance, never chosen by or visible to plugins. The engine interface is public API; third parties can ship engines without kernel changes.
...
No `bridge:storage` permission exists or is reserved. Agents never see stored bytes; a plugin that wants stored data agent-visible publishes it deliberately through the primitives that already exist, either a Command that returns it or a Context key that mirrors it, governed by the ordinary bridge grants.
...
- `observe` MUST fire synchronously on subscribe with the complete current array (the same replay behavior as `context.observe`, §8.1), and again with the complete new array on every command registration, disposal, and `when` flip, meaning a re-evaluation that changes `listed` (§11.1). Nothing else fires it.
...
- A record is the plugin's manifest data minus `setup`, which MUST NOT be reachable through this surface, plus any unknown manifest fields, preserved verbatim (§3.3), plus the kernel-added `status`.
...
- Activation, which checks capabilities and awaits each plugin's `setup` in order (§4.2), proceeds from the call but is not awaited by the caller. `ready` (§18.2) is the settling signal.
