...
This file covers package responsibilities and versioning: what each of the ten `@switchboard-dev/*` packages (plus the two example apps) is responsible for, which boundaries exist, which spec section defines each boundary, and the four independent version numbers. Runtime message flow is covered in [`bridge-flows.md`](./bridge-flows.md); execution environments and process boundaries are covered in [`topology.md`](./topology.md).
...
| `adapter-next` | server entry (`register` for `instrumentation.ts`) and client entry (bootstrap + the `<SwitchboardDev load>` loader); both entry points connect through the bridge port ([adapter contract §11](../spec/adapter-contract.md#11-switchboard-devadapter-next-normative-binding)) | same |
...
| `plugin-feedback` | annotations and the human → agent → human loop, with the reference walkthrough in [`feedback-loop.md`](./feedback-loop.md) ([brief](../spec/plugins/feedback.md)) | |
...
Each seam between parties has one source spec:
...
| plugin ↔ plugin, **data** | Events and Context, wire-safe JSON | [kernel §7](../spec/kernel-api.md#7-events)–[§8](../spec/kernel-api.md#8-context), [§14](../spec/kernel-api.md#14-the-wire-legal-rule) |
...
Four independent numbers, each covering a different compatibility gate and each with its own failure mode ([spec index, "Versions"](../spec/README.md#versions)):
...
The kernel API version also travels in the wire handshake for diagnostics only; it is not a compatibility gate ([bridge §2](../spec/bridge-protocol.md#2-versioning-the-handshake-gate)). The plugin manifest's `version` field is informational (inspector display, bug reports), and there is deliberately no `manifestVersion`: the manifest schema versions with the kernel API ([kernel §3.1](../spec/kernel-api.md#31-defineplugin-and-the-manifest)).
...
- **Unknown fields are tolerated**: unknown manifest fields, permission strings, activation hints, wire message types and fields, and toolbar contribution fields are carried with a dev-mode warning and never imply a grant.
- **Malformed input is rejected loudly**, blocking only the affected plugin (or contribution, or message).
